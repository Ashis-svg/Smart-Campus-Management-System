...
import { registerStudent } from '../../api/registerApi';
...
const StudentRegister = () => {
  ...
  const [form, setForm] = useState({ reg_no: '', email: '', password: '', confirmPassword: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.reg_no.trim() || !form.email.trim() || !form.password) {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex items-center justify-center px-4 font-sans">
      <div className="w-full max-w-sm bg-white border border-slate-200 rounded-2xl shadow-sm p-8">
        <h1 className="text-xl font-bold text-[#1e3a5f] mb-1">Student Registration</h1>
        <p className="text-sm text-slate-400 mb-6">
          Use the registration number and email on file with your hall.
        </p>

        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <label className={labelCls}>Registration Number</label>
            <input name="reg_no" className={inputCls} value={form.reg_no} onChange={handleChange} placeholder="e.g. 24U10643" />
          </div>
          <div>
        ...
        <p className="text-sm text-slate-400 mt-5 text-center">
          Already registered?{' '}
          <Link to="/student/login" className="text-blue-600 font-semibold hover:underline">Log in</Link>
        </p>
      </div>
    </div>
  );

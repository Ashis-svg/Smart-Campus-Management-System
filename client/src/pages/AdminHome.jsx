...
const ADMIN_SECTIONS = [
  ...
  {
    title: "Campus Opinions",
    // No role gate — every admin gets this module no matter which
    // role(s) they hold, so it's excluded from the role-based filter below.
    universal: true,
    icon: "💬",
    description: "Share and browse anonymous campus opinions.",
    path: "opinion",
...
const AdminHome = () => {
  ...
  const visibleSections = ADMIN_SECTIONS.filter(
    (section) => section.universal || admin.roles.includes(section.role)
  );
